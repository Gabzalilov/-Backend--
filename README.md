...
Перед публикацией остаётся перевыпустить использованные локально секреты и убедиться, что
`.env` не попал в commit. Разработчик должен уметь объяснить каждое архитектурное решение:
AI ускорял работу, но ответственность за итоговый код остаётся у автора.
